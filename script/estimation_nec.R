...
for (s in 1:nsubs) {
  ...
  ntrials_all[s] <- length(state_raw[data_g0$`Subject ID`==subIDs[s]])
  
  #pad trials with NA if n trials < maximum (i.e. 100)
  state_sub <- state_raw[data_g0$`Subject ID`==subIDs[s]] 
  length(state_sub) <- ntrials_max
  
  X_sub <- X_raw[data_g0$`Subject ID`==subIDs[s]] 
