...
for (s in 1:nsubs) {
  ...
  ntrials_all[s] <- length(state_raw[data_nec$`Subject ID`==subIDs[s]])
  
  #pad trials with NA if n trials < maximum (i.e. 100)
  state_sub <- state_raw[data_nec$`Subject ID`==subIDs[s]] 
  length(state_sub) <- ntrials_max
  
  X_sub <- X_raw[data_nec$`Subject ID`==subIDs[s]] 
...
# T-test for estimated parameters

library(readr)
df <- read_csv("JiaqiZhao#1783/Asch social conformity/Parameter_estimation.csv")
View(Parameter_estimation)

# alpha
alpha_ec <- c(rnorm(20, mean = df$`mean(alpha)`[1], sd = df$`sd(alpha)`[1]))
alpha_nec <- c(rnorm(18, mean = df$`mean(alpha)`[2], sd = df$`sd(alpha)`[2]))

t.test(alpha_ec, alpha_nec, paired = FALSE)

# beta
beta_ec <- c(rnorm(20, mean = df$`mean(beta)`[1], sd = df$`sd(beta)`[1]))
beta_nec <- c(rnorm(18, mean = df$`mean(beta)`[2], sd = df$`sd(beta)`[2]))

t.test(beta_ec, beta_nec, paired = FALSE)

# gamma
gamma_ec <- c(rnorm(20, mean = df$`mean(gamma)`[1], sd = df$`sd(gamma)`[1]))
gamma_nec <- c(rnorm(18, mean = df$`mean(gamma)`[2], sd = df$`sd(gamma)`[2]))

t.test(gamma_ec, gamma_nec, paired = FALSE)

# eta
eta_ec <- c(rnorm(20, mean = df$`mean(eta)`[1], sd = df$`sd(eta)`[1]))
eta_nec <- c(rnorm(18, mean = df$`mean(eta)`[2], sd = df$`sd(eta)`[2]))

t.test(eta_ec, eta_nec, paired = FALSE)
